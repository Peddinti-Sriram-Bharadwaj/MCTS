...
class GameState(ABC):
    ...
    def current_player(self) -> str:
        """Return whose turn it is ("X", "O" or game-specific 
        identifier)."""
        pass
    ...
    def apply_action(self, action) -> "GameState":
        """Return a new GameState with the action applied. Do not 
        mutate self."""
        pass
    ...
    def reward(self) -> bool:
        ...
        meaningful when is_terminal() is ture. Convention: negative if the 
        previous player won."""
        pass
    ...
    @abstractmethod
    def to_array(self) -> jnp.array:
        """Encode the board as a JAX array for neural network input. 
        Shape and dtype are game-specific but msut be consistent."""
        pass
