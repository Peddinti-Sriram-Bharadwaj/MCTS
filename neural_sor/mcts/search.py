...
def mcts_search(
    ...
    # Extract the maximum Q-value of the root node's children (from the root's perspective)
    # Child total_value is from the opponent's perspective, so we negate it.
    valid_children = [child for child in root.children.values() if child.visit_count > 0]
    if valid_children:
        max_q = max(-(child.total_value / child.visit_count) for child in valid_children)
    else:
        max_q = 0.0

    return best_action, pi, max_q
